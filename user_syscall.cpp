...
extern "C" void blockos_syscall_dispatch_frame(BlockOSSyscallFrame* f){
    ...
    case blockos::syscall::SYS_openat:{
        ...
        if(blockos::proc::is_proc_path(path)) blockos::proc::refresh(p);
        if(blockos::proc::is_directory_path(path,p)){ fs=fdtable(); fs[fd]={true,process::RuntimeFd::Directory,(uint16_t)f->rdx,0,(const uint8_t*)store_directory_path(path),0,0}; r=fd; break; }
    ...
    case blockos::syscall::SYS_getdents64:{
        ...
        if(!paging::is_user_range(f->rsi,(size_t)f->rdx,true)){r=EFAULT;break;}
        auto& d=fs[f->rdi]; const char* dir=(const char*)(uintptr_t)d.data;
        if(blockos::proc::is_proc_path(dir)) blockos::proc::refresh(p);
        bool proc_dir=blockos::proc::is_proc_path(dir);
        size_t count=proc_dir?blockos::proc::directory_entry_count(dir,p):vfs::directory_entry_count(dir); size_t idx=(size_t)d.off; size_t used=0;
        while(idx<count){ const char* nm=proc_dir?blockos::proc::directory_entry_name(dir,idx,p):vfs::directory_entry_name(dir,idx); if(!nm) break; size_t nl=strlen(nm); size_t reclen=(24+nl+1+7)&~(size_t)7; if(used+reclen>(size_t)f->rdx) break; uint8_t* out=(uint8_t*)(uintptr_t)(f->rsi+used); memset(out,0,reclen); uint64_t ino=idx+2; memcpy(out,&ino,8); int64_t off=(int64_t)(idx+1); memcpy(out+8,&off,8); uint16_t rr=(uint16_t)reclen; memcpy(out+16,&rr,2); char child[256]; size_t dl=strlen(dir); if(dl==1 && dir[0]=='/') snprintf(child,sizeof(child),"/%s",nm); else snprintf(child,sizeof(child),"%s/%s",dir,nm); bool child_dir=blockos::proc::is_proc_path(child)?blockos::proc::is_directory_path(child,p):vfs::is_directory(child); out[18]=child_dir?4:8; memcpy(out+19,nm,nl+1); used+=reclen; ++idx; } d.off=idx; r=(int64_t)used; break; }
    ...
    case blockos::syscall::SYS_statx:{
        char path[256]; if(!copy_user_string(f->rdi,path,sizeof(path))){r=EFAULT;break;} char abs[256];if(!resolve_path_for_process(p,path,abs,sizeof(abs))){r=ENAMETOOLONG;break;} if(blockos::proc::is_proc_path(abs)) blockos::proc::refresh(p); uint32_t sz=0; const uint8_t*d=vfs::read_file(abs,&sz); bool dir=blockos::proc::is_proc_path(abs)?blockos::proc::is_directory_path(abs,p):vfs::is_directory(abs); if(!d&&!dir){r=ENOENT;break;} if(!paging::is_user_range(f->r9,256,true)){r=EFAULT;break;} memset((void*)(uintptr_t)f->r9,0,256); ((uint64_t*)(uintptr_t)f->r9)[0]=1; ((uint64_t*)(uintptr_t)f->r9)[2]=dir?0040755:0100444; ((uint64_t*)(uintptr_t)f->r9)[11]=sz; r=0;break; }
    ...
    case blockos::syscall::SYS_access:{char path[256];if(!copy_user_string(f->rdi,path,sizeof(path))){r=EFAULT;break;}if(blockos::proc::is_proc_path(path)) blockos::proc::refresh(p);uint32_t sz=0;bool ok=blockos::proc::is_proc_path(path)?(vfs::read_file(path,&sz)||blockos::proc::is_directory_path(path,p)):(vfs::read_file(path,&sz)||vfs::is_directory(path)||vfs::is_device(path));r=ok?0:ENOENT;break;}
    case blockos::syscall::SYS_stat: case blockos::syscall::SYS_lstat:{
        ...
        if(blockos::proc::is_proc_path(path)) blockos::proc::refresh(p);
        uint32_t sz=0;const uint8_t*d=vfs::read_file(path,&sz);bool dir=blockos::proc::is_proc_path(path)?blockos::proc::is_directory_path(path,p):vfs::is_directory(path);bool dev=vfs::is_device(path);
