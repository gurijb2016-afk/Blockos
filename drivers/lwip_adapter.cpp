...
void lwip_adapter::init() {
    ...
    if (!virtio_net::init()) {
        Print((CHAR16*)L"lwip_adapter: virtio-net not available, lwIP cannot be attached\n");
        return;
    }
    Print((CHAR16*)L"lwip_adapter: initialized (adapter only)\n");
}
...
void lwip_adapter::poll_receive() {
    ...
    if (r > 0) {
        ...
        Print((CHAR16*)L"lwip_adapter: received packet (len)\n");
    }
}
