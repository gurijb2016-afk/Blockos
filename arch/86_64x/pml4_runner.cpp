...
void run_in_address_space(uint64_t pml4_phys, void (*entry)(void)) {
    ...
    UnicodeSPrint(buf, sizeof(buf), (CHAR16*)L"pml4_runner: switching CR3 0x%016lx -> 0x%016lx\n", old_cr3, pml4_phys);
    Print(buf);
    ...
    __asm__ volatile ("mov %%cr3, %0" : "=r" (old_cr3));
    UnicodeSPrint(buf, sizeof(buf), (CHAR16*)L"pml4_runner: restored CR3 to 0x%016lx\n", old_cr3);
    Print(buf);
}
...
bool run_elf_in_address_space(const void* elf_buf, size_t elf_size) {
    ...
    if (!elf_loader::load_elf64_from_mem(elf_buf, elf_size, &entry, &pml4)) {
        Print((CHAR16*)L"pml4_runner: elf_loader failed\n");
        return false;
    }

    CHAR16 buf[128];
    UnicodeSPrint(buf, sizeof(buf), (CHAR16*)L"pml4_runner: running ELF entry 0x%016lx pml4 0x%016lx\n", entry, pml4);
    Print(buf);
